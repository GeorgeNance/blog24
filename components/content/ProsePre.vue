...
<template>
	<div class="relative rounded-lg bg-[var(--shiki-default-bg)] shadow-lg ">
		<div class="flex justify-end items-center py-1 px-2 pl-3 absolute top-0 right-0">
			...
			<span v-if="codeCopied" class="px-3 rounded text-green-300 font-['Arial',sans-serif] text-sm opacity-50">
				<i>Copied</i>
			</span>
			<button class="px-3  rounded border font-['Arial',sans-serif] text-sm text-[var(--shiki-default)] opacity-25 hover:opacity-50 bg-inherit border-[var(--shiki-default)] hover:text-green-300 hover:border-emerald-300 whitespace-nowrap transition-all duration-300"
					@click="copyCode">
				Copy
			</button>
		</div>
